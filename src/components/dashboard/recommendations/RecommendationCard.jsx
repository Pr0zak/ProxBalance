        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            {rec.mount_point_info?.has_mount_points && (
              ...
              } text-pb-text dark:text-white text-[10px] font-bold rounded`}
              ...
              <>
                ...
                <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded font-semibold">
                  ...
                  {rec.trend_evidence?.available && (() => {
                    ...
                    if (dir === 'falling' || dir === 'sustained_decrease') return <TrendingDown size={10} className="text-green-500 ml-0.5" title="CPU falling" />;
                    return <Minus size={10} className="text-pb-text2 dark:text-gray-400 ml-0.5" title="CPU stable" />;
                  })()}
                </span>
                {rec.score_improvement !== undefined && (() => {
                  ...
                  return (
                    <span className="inline-flex items-center gap-1.5 min-w-[120px]" title={`Score improvement: +${rec.score_improvement.toFixed(1)} penalty points`}>
                      <span className="text-xs text-pb-text2 dark:text-gray-400">+{rec.score_improvement.toFixed(0)}</span>
          ...
          {rec.decision_explanation && (
            <div className="mt-1.5 text-xs text-pb-text2 dark:text-gray-400 italic">
              {rec.decision_explanation}
            </div>
          )}
              ...
              {collapsedSections[`details-${idx}`] && (
                <div className="mt-2 p-3 bg-gradient-to-r from-indigo-900/20 to-blue-900/20 border border-indigo-300 dark:border-indigo-700 rounded text-xs space-y-3">
                    ...
                    <div>
                      <div className="font-semibold text-red-600 dark:text-red-400 mb-1 flex items-center gap-1">
                        ...
                        {rec.trend_evidence?.available && (() => {
                          ...
                          return <Minus size={12} className="text-pb-text2 dark:text-gray-400" />;
                        })()}
                      </div>
                      <div className="space-y-0.5 text-pb-text2 dark:text-gray-400">
                        {rec.score_details && (
                          <>
                            <div>Score: {rec.score_details.source?.total_score?.toFixed(1) || 'N/A'}</div>
                            <div className="text-[10px] mt-1 font-medium text-pb-text2 dark:text-gray-500">Penalties:</div>
                      ...
                      <div className="space-y-1">
                        {rec.trend_evidence.decision_factors.map((f, i) => (
                          <div key={i} className="flex items-start gap-2">
                            ...
                            <span className="text-pb-text2 dark:text-gray-400">
                              {f.factor}
                              {f.weight === 'high' && <span className="ml-1 text-[9px] font-bold text-pb-text2 dark:text-gray-500">(HIGH)</span>}
                            </span>
                          </div>
                        ))}
                      </div>
                  ...
                  {rec.trend_evidence?.data_quality && (
                    <div className="text-[10px] text-pb-text2 dark:text-gray-500 pt-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {canMigrate && (
                    <button
                      ...
                      className="px-3 py-2 text-xs bg-pb-surface2 dark:bg-slate-700 text-pb-text dark:text-gray-300 rounded hover:bg-gray-600 disabled:opacity-50 flex items-center gap-1 transition-colors"
