      ...
      {hasForecasts && (
        <div className="mb-4">
          <button
            ...
            className="flex items-center gap-2 text-sm font-medium text-pb-text dark:text-gray-300 hover:text-pb-text dark:hover:text-white transition-colors mb-2"
          >
            ...
            <span className="text-xs text-pb-text2 dark:text-gray-500">— Projected threshold crossings</span>
          </button>
              ...
              {recommendationData.forecasts.map((fc, idx) => (
                <div key={idx} className={`flex items-start gap-3 p-3 rounded-xl border ${
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-pb-text2 dark:text-gray-400 mb-1">{fc.message}</p>
                    <div className="flex flex-wrap gap-3 text-[10px] text-pb-text2 dark:text-gray-500">
                      <span>Current: <strong className="text-pb-text dark:text-gray-300">{fc.current_value?.toFixed(1)}%</strong></span>
                      <span>Threshold: <strong className="text-pb-text dark:text-gray-300">{fc.threshold}%</strong></span>
                      <span>Projected: <strong className="text-pb-text dark:text-gray-300">{fc.projected_value?.toFixed(1)}%</strong></span>
                      {fc.estimated_hours_to_crossing && <span>ETA: <strong className="text-pb-text dark:text-gray-300">~{fc.estimated_hours_to_crossing < 24 ? `${fc.estimated_hours_to_crossing.toFixed(0)}h` : `${(fc.estimated_hours_to_crossing / 24).toFixed(1)}d`}</strong></span>}
                      <span>Rate: <strong className="text-pb-text dark:text-gray-300">{fc.trend_rate_per_day > 0 ? '+' : ''}{fc.trend_rate_per_day?.toFixed(1)}%/day</strong></span>
                      <span>Confidence: <strong className="text-pb-text dark:text-gray-300">{fc.confidence}</strong> (R²={fc.r_squared?.toFixed(2)})</span>
                    </div>
                  </div>
                </div>
              ))}
