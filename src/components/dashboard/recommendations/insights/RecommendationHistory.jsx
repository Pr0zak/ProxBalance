  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <select value={hours} onChange={e => setHours(Number(e.target.value))} className="text-[10px] px-1.5 py-1 rounded border border-slate-600 bg-slate-700 text-gray-400">
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center gap-0.5 group relative" title={`${timeLabel}\nHealth: ${healthValues[i].toFixed(0)}%\nRecs: ${recCounts[i]}`}>
              <div className="w-full flex flex-col justify-end h-16">
                <div className="w-full bg-green-500 rounded-t-sm opacity-60 group-hover:opacity-100 transition-opacity" style={{ height: `${healthPct}%`, minHeight: healthPct > 0 ? '1px' : '0' }}></div>
              </div>
      ...
      <div className="mt-2 text-[10px] text-gray-400">
        {entries.length} snapshots over last {hours}h — Latest health: <strong className="text-gray-300">{healthValues[healthValues.length - 1]?.toFixed(0)}%</strong>, Recs: <strong className="text-gray-300">{recCounts[recCounts.length - 1]}</strong>
      </div>
    </div>
  );
}
