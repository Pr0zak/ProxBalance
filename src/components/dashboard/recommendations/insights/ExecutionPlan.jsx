...
export default function ExecutionPlan({ recommendationData }) {
  ...
  if (!plan?.ordered_recommendations?.length || plan.ordered_recommendations.length <= 1) {
    return (
      <div className="text-xs text-pb-text2 dark:text-gray-500 py-4">
    ...
    <div className="space-y-2">
      ...
      <div className="space-y-1.5">
        {plan.ordered_recommendations.map((step, idx) => (
          <div key={idx} className="flex items-center gap-2 text-xs p-2 bg-gray-800/50 rounded border border-pb-border dark:border-slate-700">
            ...
            <div className="flex-1 min-w-0">
              ...
              <span className="text-pb-text2 dark:text-gray-500 mx-1">{step.source_node}</span>
              <ArrowRight size={10} className="inline text-pb-text2 dark:text-gray-400" />
              <span className="text-pb-text2 dark:text-gray-500 mx-1">{step.target_node}</span>
            </div>
            {step.parallel_group !== undefined && (
              <span className="px-1.5 py-0.5 text-[9px] font-medium rounded bg-pb-surface2 dark:bg-slate-700 text-pb-text2 dark:text-gray-400">
                Group {step.parallel_group + 1}
              </span>
            )}
            {step.reason_for_order && (
              <span className="text-[10px] text-pb-text2 dark:text-gray-500 max-w-[200px] truncate" title={step.reason_for_order}>
                {step.reason_for_order}
              </span>
            )}
          </div>
        ...
        {plan.can_parallelize && plan.parallel_groups?.length > 0 && (
          <div className="text-[10px] text-pb-text2 dark:text-gray-500 mt-1 px-2">
