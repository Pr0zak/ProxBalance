  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
        <div className="bg-gray-900/50 rounded p-2 border border-slate-700">
          <div className="text-gray-400 mb-0.5">Generation Time</div>
          <div className="font-mono font-semibold text-white">
            {recommendationData.generation_time_ms ? `${recommendationData.generation_time_ms}ms` : 'N/A'}
          </div>
        </div>
        <div className="bg-gray-900/50 rounded p-2 border border-slate-700">
          <div className="text-gray-400 mb-0.5">Recommendations</div>
          <div className="font-mono font-semibold text-white">
            {recommendationData.count || recommendations.length}
          </div>
        </div>
        <div className="bg-gray-900/50 rounded p-2 border border-slate-700">
          <div className="text-gray-400 mb-0.5">Guests Evaluated</div>
          <div className="font-mono font-semibold text-white">
            {(recommendationData.count || 0) + (recommendationData.skipped_guests?.length || 0)}
          </div>
        </div>
        <div className="bg-gray-900/50 rounded p-2 border border-slate-700">
          <div className="text-gray-400 mb-0.5">Skipped</div>
          <div className="font-mono font-semibold text-white">
            {recommendationData.skipped_guests?.length || 0}
          </div>
        </div>
        <div className="bg-gray-900/50 rounded p-2 border border-slate-700">
          <div className="text-gray-400 mb-0.5">AI Enhanced</div>
          <div className={`font-semibold ${recommendationData.ai_enhanced ? 'text-purple-400' : 'text-gray-400'}`}>
            {recommendationData.ai_enhanced ? 'Yes' : 'No'}
          </div>
        </div>
        <div className="bg-gray-900/50 rounded p-2 border border-slate-700">
          <div className="text-gray-400 mb-0.5">Conflicts / Advisories</div>
          <div className="font-mono font-semibold text-white">
      ...
      {recommendationData.parameters && (
        <div className="p-2 bg-gray-900/50 rounded border border-slate-700 text-xs">
      ...
      {recommendationData.summary?.skip_reasons && Object.keys(recommendationData.summary.skip_reasons).length > 0 && (
        <div className="p-2 bg-gray-900/50 rounded border border-slate-700 text-xs">
          <span className="text-gray-400 block mb-1">Skip Reasons:</span>
          <div className="flex flex-wrap gap-2">
            {Object.entries(recommendationData.summary.skip_reasons).map(([reason, count]) => (
              <span key={reason} className="px-1.5 py-0.5 bg-gray-800 rounded text-gray-300 font-mono">
                {reason}: {count}
              </span>
            ))}
          </div>
