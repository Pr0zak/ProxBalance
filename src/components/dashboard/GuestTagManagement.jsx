            ...
            <div className="flex items-center justify-between gap-2 mb-3 sm:mb-6">
              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h2 className="text-base sm:text-2xl font-bold text-white">Guest Tag Management</h2>
                  <p className="text-xs sm:text-sm text-gray-400 mt-0.5 hidden sm:block">Manage ignore tags and affinity rules for all guests</p>
                </div>
              </div>
              <button
                onClick={() => toggleSection('taggedGuests')}
                className="p-1.5 sm:p-2 hover:bg-slate-700 rounded-lg transition-all duration-200 shrink-0"
                title={collapsedSections.taggedGuests ? "Expand section" : "Collapse section"}
              >
                <ChevronDown size={ICON.section} className={`text-gray-400 transition-transform duration-200 ${!collapsedSections.taggedGuests ? 'rotate-180' : ''}`} />
              </button>
            </div>

            {collapsedSections.taggedGuests ? (
              <div className="grid grid-cols-3 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
                <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-gray-700/50 border border-slate-600 rounded">
                  <HardDrive size={16} className="text-gray-400 shrink-0 sm:hidden" />
                  <HardDrive size={18} className="text-gray-400 shrink-0 hidden sm:block" />
                  <div className="min-w-0">
                    <div className="font-semibold text-sm sm:text-base text-white truncate">Total</div>
                    <div className="text-xs sm:text-sm text-gray-400">{Object.keys(data.guests).length}</div>
                  </div>
                </div>
                <div className={`flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded border ${ignoredGuests.length > 0 ? 'bg-yellow-900/30 border-yellow-800' : 'bg-gray-700/30 border-slate-700 opacity-60'}`}>
                  <Shield size={16} className={`shrink-0 sm:hidden ${ignoredGuests.length > 0 ? 'text-yellow-400' : 'text-gray-500'}`} />
                  <Shield size={18} className={`shrink-0 hidden sm:block ${ignoredGuests.length > 0 ? 'text-yellow-400' : 'text-gray-500'}`} />
                  <div className="min-w-0">
                    <div className="font-semibold text-sm sm:text-base text-white truncate">Ignored</div>
                    <div className="text-xs sm:text-sm text-gray-400">{ignoredGuests.length}</div>
                  </div>
                </div>
                <div className={`flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded border ${autoMigrateOkGuests.length > 0 ? 'bg-green-900/30 border-green-800/50' : 'bg-gray-700/30 border-slate-700 opacity-60'}`}>
                  <CheckCircle size={16} className={`shrink-0 sm:hidden ${autoMigrateOkGuests.length > 0 ? 'text-green-400' : 'text-gray-500'}`} />
                  <CheckCircle size={18} className={`shrink-0 hidden sm:block ${autoMigrateOkGuests.length > 0 ? 'text-green-400' : 'text-gray-500'}`} />
                  <div className="min-w-0">
                    <div className="font-semibold text-sm sm:text-base text-white truncate">Auto-Migrate</div>
                    <div className="text-xs sm:text-sm text-gray-400">{autoMigrateOkGuests.length}</div>
                  </div>
                </div>
                <div className={`flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded border ${excludeGuests.length > 0 ? 'bg-blue-900/30 border-blue-800/50' : 'bg-gray-700/30 border-slate-700 opacity-60'}`}>
                  <Shield size={16} className={`shrink-0 sm:hidden ${excludeGuests.length > 0 ? 'text-blue-400' : 'text-gray-500'}`} />
                  <Shield size={18} className={`shrink-0 hidden sm:block ${excludeGuests.length > 0 ? 'text-blue-400' : 'text-gray-500'}`} />
                  <div className="min-w-0">
                    <div className="font-semibold text-sm sm:text-base text-white truncate">Anti-Affinity</div>
                    <div className="text-xs sm:text-sm text-gray-400">{excludeGuests.length}</div>
                  </div>
                </div>
                <div className={`flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded border ${affinityGuests.length > 0 ? 'bg-purple-900/30 border-purple-800' : 'bg-gray-700/30 border-slate-700 opacity-60'}`}>
                  <Shield size={16} className={`shrink-0 sm:hidden ${affinityGuests.length > 0 ? 'text-purple-400' : 'text-gray-500'}`} />
                  <Shield size={18} className={`shrink-0 hidden sm:block ${affinityGuests.length > 0 ? 'text-purple-400' : 'text-gray-500'}`} />
                  <div className="min-w-0">
                    <div className="font-semibold text-sm sm:text-base text-white truncate">Affinity</div>
                    <div className="text-xs sm:text-sm text-gray-400">{affinityGuests.length}</div>
                  </div>
                </div>
              </div>
            ) : (
                    ...
                    <thead>
                      <tr className="border-b border-slate-700">
                        <th
                          ...
                          className={`${TABLE_HEADER} cursor-pointer hover:bg-slate-700/50`}
                        >
                          <div className="flex items-center gap-1">
                            Type
                            {guestSortField === 'type' && (
                              <span className="text-blue-400 font-bold">{guestSortDirection === 'asc' ? '\u2191' : '\u2193'}</span>
                            )}
                          </div>
                        </th>
                        <th
                          ...
                          className={`hidden sm:table-cell ${TABLE_HEADER} cursor-pointer hover:bg-slate-700/50`}
                        >
                          <div className="flex items-center gap-1">
                            ID
                            {guestSortField === 'vmid' && (
                      ...
                      >
                        <td className="p-3">
                          <span className={`px-2 py-1 text-xs rounded font-medium ${
                            guest.type === 'VM' ? 'bg-blue-900/30 text-blue-300' :
                            'bg-purple-900/30 text-purple-300'
                          }`}>
                            {guest.type}
                          </span>
                        </td>
                        <td className="hidden sm:table-cell p-3 text-sm font-mono text-white">{guest.vmid}</td>
                        <td className="p-3">
                          ...
                          {guestHasTags && (
                            <div className="flex flex-wrap gap-1 mt-1 sm:hidden">
                              {guest.tags.has_ignore && (
                                <span className="text-[10px] px-1.5 py-0.5 bg-yellow-800 text-yellow-200 rounded font-medium">
                                  ignore
                                </span>
                              )}
                              ...
                              {guest.tags.exclude_groups?.map(tag => (
                                <span key={tag} className="text-[10px] px-1.5 py-0.5 bg-blue-800 text-blue-200 rounded font-medium">
                                  {tag}
                                </span>
                              ))}
                              {guest.tags.affinity_groups?.map(tag => (
                                <span key={tag} className="text-[10px] px-1.5 py-0.5 bg-purple-800 text-purple-200 rounded font-medium">
                                  {tag}
                                </span>
                              ))}
                            </div>
                        ...
                        <td className="p-3 text-sm text-gray-400">{guest.node}</td>
                        <td className="p-3">
                          <div className="flex items-center gap-1.5">
                            <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs rounded font-medium ${
                              guest.status === 'migrating' ? 'bg-blue-900/30 text-blue-300' :
                              guest.status === 'running' ? 'bg-green-900/30 text-green-300' :
                        ...
                        <td className="hidden sm:table-cell p-3">
                          <div className="flex flex-wrap gap-1">
                            {guest.tags.has_ignore && (
                              <span className="inline-flex items-center gap-1 text-xs px-2 py-1 bg-yellow-800 text-yellow-200 rounded font-medium">
                                ignore
                                {canMigrate && (
                                  <button
                            ...
                            {guest.tags.all_tags?.includes('auto_migrate_ok') && (
                              <span className="inline-flex items-center gap-1 text-xs px-2 py-1 bg-green-800 text-green-200 rounded font-medium">
                                auto_migrate_ok
                                {canMigrate && (
                                  <button
                            ...
                            {guest.tags.exclude_groups.map(tag => (
                              <span key={tag} className="inline-flex items-center gap-1 text-xs px-2 py-1 bg-blue-800 text-blue-200 rounded font-medium">
                                {tag}
                                {canMigrate && (
                                  <button
