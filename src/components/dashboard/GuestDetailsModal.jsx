      ...
      <div className={`${MODAL_CONTAINER.replace('max-w-md', 'max-w-3xl')} !rounded-t-xl sm:!rounded-2xl !max-h-[85vh] sm:!max-h-[90vh] flex flex-col !overflow-hidden`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-700 shrink-0">
          ...
          <button
            ...
            className="ml-2 shrink-0 p-1.5 rounded-lg text-gray-400 hover:text-gray-200 hover:bg-gray-700"
            aria-label="Close"
          >
            <X size={20} />
          </button>
        ...
        <div className="p-4 overflow-y-auto">
          ...
          <div className="flex items-center justify-between mb-3 pb-3 border-b border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-medium text-white">{selectedGuestDetails.currentNode}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            {/* CPU */}
            <div className="relative overflow-hidden bg-gradient-to-br from-blue-900/20 to-blue-800/20 rounded p-2 border border-blue-800">
              ...
              <div className="relative z-10">
                ...
                <div className="text-xl font-bold text-white">{(selectedGuestDetails.cpu_current || 0).toFixed(1)}%</div>
                <div className="text-[10px] text-gray-400">{selectedGuestDetails.cpu_cores || 0} cores</div>
              </div>
            </div>

            {/* Memory */}
            <div className="relative overflow-hidden bg-gradient-to-br from-purple-900/20 to-purple-800/20 rounded p-2 border border-purple-800">
              {/* Sparkline background */}
              <svg className="absolute inset-0 w-full h-full opacity-30" preserveAspectRatio="none" viewBox="0 0 100 100">
                <polyline
                  fill="none"
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            ...
            <div className="bg-cyan-900/20 rounded p-2 border border-cyan-800">
          ...
          {selectedGuestDetails.tags && selectedGuestDetails.tags.all_tags && selectedGuestDetails.tags.all_tags.length > 0 && (
            <div className="border-t border-slate-700 pt-2 mt-2">
              <div className="text-[10px] uppercase tracking-wide text-gray-400 font-medium mb-1.5">Tags</div>
              ...
              <div className="space-y-2">
                {selectedGuestDetails.mount_points.mount_points && selectedGuestDetails.mount_points.mount_points.map((mp, idx) => (
                  <div key={idx} className={`p-3 rounded-lg border ${
                    ...
                    {mp.is_bind_mount && !mp.is_shared && (
                      <div className="mt-2 pt-2 border-t border-orange-800">
                ...
                <div className="mt-3 p-3 bg-orange-900/30 border border-orange-700 rounded-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-xs text-orange-200">
                      ...
                      <p>This container has unshared bind mounts that require manual intervention. Use <span className="font-mono bg-orange-800 px-1">pct migrate {selectedGuestDetails.vmid} &lt;target&gt; --restart --force</span> and verify paths exist on target node.</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-3">
                  ...
                  <div className="space-y-2">
                    {selectedGuestDetails.local_disks.passthrough_disks.map((disk, idx) => (
                      <div key={idx} className="p-3 rounded-lg border bg-red-900/20 border-red-700">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-mono text-sm font-semibold text-white">
          ...
          <div className="border-t border-slate-700 pt-3 mt-2">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400">Score comparison across all nodes</span>
              </div>
            ...
            {guestModalCollapsed.migrationOptions && (
              <div className="space-y-2">
                {loadingGuestOptions ? (
                      ...
                      return (
                        <div key={opt.node} className={`p-2 rounded border text-xs ${
                          opt.is_current
                            ...
                            : opt.disqualified
                            ? 'border-slate-700 bg-gray-800/50 opacity-60'
                            : opt.suitable
                          ...
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <div className="text-right">
                              {opt.disqualified ? (
                                <span className="text-gray-400">{opt.reason}</span>
                              ) : (
                                <span className={`font-semibold ${
                                  opt.suitability_rating >= 70 ? 'text-green-400' :
                                  opt.suitability_rating >= 50 ? 'text-yellow-400' :
                                  opt.suitability_rating >= 30 ? 'text-orange-400' :
                                  'text-red-400'
                  ...
                  <div className="text-xs text-gray-400 p-2">No migration data available</div>
                )}
              </div>
            )}
          </div>
        ...
        <div className="flex items-center justify-end gap-3 p-6 border-t border-slate-700">
          <button
            onClick={() => setSelectedGuestDetails(null)}
            className="px-4 py-2 text-gray-300 bg-gray-700 hover:bg-gray-600 rounded font-medium flex items-center justify-center gap-1.5"
          >
            <X size={14} /> Close
          </button>
