...
export default function IconLegend({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="bg-slate-800/95 backdrop-blur-xl rounded-2xl shadow-2xl shadow-black/20 border border-claude-border dark:border-slate-700/50 max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col modal-enter"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between p-4 border-b border-claude-border dark:border-slate-700">
          <h2 className="text-lg font-bold text-claude-text dark:text-white flex items-center gap-2">
            <HelpCircle size={20} /> Icon Reference
          </h2>
          <button onClick={onClose} className="p-1 hover:bg-claude-surface2 dark:hover:bg-slate-700 rounded" title="Close">
        ...
        <div className="px-4 pt-3 pb-2">
          <div className="relative">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-claude-muted dark:text-gray-400" />
            <input
              type="text"
              placeholder="Search icons..."
              value={searchTerm}
              onChange={e => setSearchTerm(e.target.value)}
              className="w-full pl-9 pr-3 py-2 text-sm border border-claude-border dark:border-slate-600 rounded-lg bg-claude-surface2 dark:bg-gray-700 text-claude-text dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500"
            />
          </div>
        </div>
        ...
        <div className="overflow-y-auto p-4 pt-2 space-y-4">
          {filteredGroups.map(group => (
            <div key={group.label}>
              <h3 className="text-xs font-semibold uppercase tracking-wider text-claude-muted dark:text-gray-400 mb-2">{group.label}</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                {group.icons.map(({ Icon, name }) => (
                  ...
                  >
                    <Icon size={16} className="text-claude-text dark:text-gray-300 flex-shrink-0" />
                    <span className="text-claude-text dark:text-gray-200 truncate">{name}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {filteredGroups.length === 0 && (
            <p className="text-center text-claude-muted dark:text-gray-400 py-4">No icons match your search.</p>
          )}
        </div>
      </div>
    </div>
