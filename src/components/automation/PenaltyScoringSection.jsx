    ...
    <div id="penalty-config-section" className={outerClass}>
      ...
      {!collapsedSections.penaltyScoring && (
        <div className="space-y-4">
          ...
          {/* Trend Weight */}
          <div>
            <label className="block text-sm font-medium text-pb-text dark:text-gray-300 mb-1">Trend Weight</label>
            <p className="text-xs text-pb-text2 dark:text-gray-400 mb-3">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-pb-text2 dark:text-gray-400 whitespace-nowrap w-16">Snapshot</span>
              <input
                type="range"
                min={0}
                max={100}
              ...
              />
              <span className="text-xs text-pb-text2 dark:text-gray-400 whitespace-nowrap w-12 text-right">Trends</span>
              <span className="text-xs font-mono text-pb-text dark:text-gray-300 w-10 text-right">{settings.trend_weight}%</span>
            </div>
          </div>

          {/* Analysis Lookback */}
          <div>
            <label className="block text-sm font-medium text-pb-text dark:text-gray-300 mb-1">Analysis Lookback</label>
            <p className="text-xs text-pb-text2 dark:text-gray-400 mb-3">
            ...
            <div className="mt-3 space-y-4">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <label className="block text-sm font-medium text-pb-text dark:text-gray-300">Min Score Improvement</label>
                ...
                <p className="text-xs text-pb-text2 dark:text-gray-400 mb-2">
              ...
              <div className="p-4 bg-pb-surface2 dark:bg-slate-700 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <label className="block text-sm font-medium text-pb-text dark:text-gray-300">What-If Simulator</label>
                    <p className="text-xs text-pb-text2 dark:text-gray-400">Preview how your current settings would affect recommendations before saving.</p>
                  </div>
                  <button
                  ...
                  <div className="mt-3 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-3 bg-white dark:bg-slate-800 rounded-lg text-center">
                        <div className="text-xs text-pb-text2 dark:text-gray-400 mb-1">Current Settings</div>
                        <div className="text-2xl font-bold text-pb-text dark:text-white">{simulatorResult.current_count}</div>
                    ...
                    {simulatorResult.changes && simulatorResult.changes.length > 0 && (
                      <div>
                        <h5 className="text-xs font-semibold text-pb-text dark:text-gray-300 mb-1.5">Changes</h5>
                        <div className="space-y-1 max-h-40 overflow-y-auto">
                          {simulatorResult.changes.map((change, i) => (
                            <div key={i} className="flex items-center gap-2 text-xs p-1.5 bg-white dark:bg-slate-800 rounded-lg">
                              ...
                              <span className="text-pb-text dark:text-gray-300">
                                {change.type} {change.vmid} ({change.name})
                              </span>
                              <span className="text-pb-text2 dark:text-gray-400 ml-auto">
                    ...
                    {simulatorResult.node_score_comparison && (
                      <div>
                        <h5 className="text-xs font-semibold text-pb-text dark:text-gray-300 mb-1.5">Node Score Impact</h5>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                          {Object.entries(simulatorResult.node_score_comparison).map(([node, scores]) => (
                            <div key={node} className="flex items-center justify-between text-xs p-1.5 bg-white dark:bg-slate-800 rounded-lg">
                              <span className="font-medium text-pb-text dark:text-gray-300">{node}</span>
                    ...
                    {simulatorResult.changes?.length === 0 && simulatorResult.current_count === simulatorResult.proposed_count && (
                      <p className="text-xs text-pb-text2 dark:text-gray-400 text-center py-2">
                ...
                <>
                  <div className="space-y-3 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div>
                        <label className="block text-xs text-pb-text dark:text-gray-300 mb-1">
                  ...
                  <div className="space-y-3">
                    <label className="block text-sm font-medium text-pb-text dark:text-gray-300">CPU Penalties</label>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  {/* Memory Penalties */}
                  <div className="space-y-3">
                    <label className="block text-sm font-medium text-pb-text dark:text-gray-300">Memory Penalties</label>
                    <p className="text-xs text-pb-text2 dark:text-gray-400">
                  ...
                  {/* IOWait Penalties */}
                  <div className="space-y-3">
                    <label className="block text-sm font-medium text-pb-text dark:text-gray-300">IOWait Penalties</label>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
